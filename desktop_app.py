...
from PyQt5.QtWidgets import (
    ...
    QFormLayout, QGroupBox, QFrame, QCheckBox,
    QStatusBar,
)
...
class LoginDialog(QDialog):
    ...
    def update_tab_styles(self):
        ...
        self.login_tab.setStyleSheet(
            active_style if self.login_tab.isChecked() else inactive_style
        )
        self.register_tab.setStyleSheet(
            active_style if self.register_tab.isChecked() else inactive_style
        )
...
class TodoItemWidget(QWidget):
    ...
    def setup_ui(self, todo):
        ...
        delete_btn.clicked.connect(
            lambda: self.deleted.emit(self.todo_id)
        )
...
class AddTodoDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.priority_combo.setCurrentText(
            self.todo.get('priority', 'medium').capitalize()
            if self.todo else "Medium"
        )
        ...
        save_btn = QPushButton(
            "Save Task" if self.todo else "Create Task"
        )
...
class SkyyMainWindow(QMainWindow):
    ...
    def load_todos(self):
        ...
        try:
            # Get todos via API
            self.api_client.session.get(
                f"{self.api_client.base_url}/dashboard"
            )
